          ...
          <ul class="normalTitle">
            ...
            <li>贷款状态：
              <span>{{UserList.loanStatus}}</span>
                ...
                <el-button  slot="button-content" type="primary" @click="cancelRequest">取消申请</el-button> -->
		        </div>
          </ul>
        </div>
      </div>
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="挂账撤销金额" class="left_form_item" prop="applyAmount">
               <el-input v-model="formData.applyAmount" placeholder="请输入" v-bind:style="inputStyle" @change="revocationMoney()" disabled="disabled"></el-input>元
            </el-form-item>
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="申请备注" class="left_form_item" prop="remark">
               <el-input v-model="formData.remark" placeholder="请输入" v-bind:style="inputStyle" type="textarea" disabled="disabled"></el-input>
            </el-form-item>
...
<script>
 ...
 // import searchLabels from './list/searchLabel'
  ...
  import {reAcctinfo,repealSubmit,detailRepeal} from '@/api/carLoadAfterApi'
  ...
  export default {
    ...
    methods: {
      ...
      goList(){
        this.$router.push({name:'revocationRegister',query:{activeName:'second'}})
      }
