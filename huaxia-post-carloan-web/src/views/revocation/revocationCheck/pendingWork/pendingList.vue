...
<script>
  ...
  import {refhTodoTasks,reCancel,reSubmit4FH} from '@/api/carLoadAfterApi'
  ...
  export default {
    ...
    methods: {
      ...
      /* 检索 */
      getSearch(param) {
        refhTodoTasks(param).then(response => {
        	//alert("搜索");
      ...
      contractNoClick(data) {
        let list = cloneDeep(this.tableParam.tableData),
          ...
          pushUrl = ""
        for (let i in list) {
          if (list[i].contractNo === contractNo) {
            index = i
          }
          if (i >= index) {
       ...
       // sessionStorage.caseCheckList = JSON.stringify(tempArray);
        sessionStorage.revocationCheckData = JSON.stringify(caseData);//合同号+流水号

        pushUrl = "/revocation/revocationCheck/pendingWork/pendingInfo";
      ...
      addDataList(){
        let oSubmitData = {
      	...
      	reSubmit4FH(oSubmitData).then(response => {
          if (response.data.code === "00000") {
            this.$message({
              message: '提交成功',
              type: 'success'
