...
<script>
  ...
  import {initialAuditHandled} from '@/api/carLoadAfterApi'
  ...
  export default {
    ...
    methods: {
			//检索
      getSearch(param) {
        initialAuditHandled(param).then(response => {
        	//alert("搜索");
...
/*合同编号点击事件*/
      contractNoClick(data) {
        ...
        sessionStorage.publicInitialData = JSON.stringify(caseData);//合同号+流水号
				...
				//跳转详情页面 
        pushUrl = "/publicRepayment/initialAudit/handledWork/handledInfo";
