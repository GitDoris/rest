...
<script>
  ...
  import {editMatch,goback,mRecords,actualCalculate,payInfo,actualPayInfo,contractInfo,imgePlatformUrl,cancel,submit,save,corPayInfo,dgEarlyClearanceInfo} from '@/api/carLoadAfterApi'
...
// this.caseCheckList = JSON.parse(sessionStorage.caseCheckList);
    ...
    methods: {
      ...
      //查询测算结果
        ...
        let param = {
          amraId:this.serialNo2
    		...
    		actualCalculate(param).then(response => {
      ...
      goList(){
        this.$router.push({name:'lastAudit',query:{activeName:'third'}})
      }
