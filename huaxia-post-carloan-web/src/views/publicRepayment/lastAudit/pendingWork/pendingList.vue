...
<script>
  ...
  import {finalAuditPending,initialAuditSubmit} from '@/api/carLoadAfterApi'
  ...
  export default {
    ...
    methods: {
      ...
      /* 检索 */
      getSearch(param) {
        finalAuditPending(param).then(response => {
        	//alert("搜索");
      ...
      contractNoClick(data) {
        ...
        sessionStorage.publicLastData = JSON.stringify(caseData);//合同号+流水号

				//跳转详情页面  路由传参数跳转详情
        pushUrl = "/publicRepayment/lastAudit/pendingWork/pendingInfo";
