...
import store from '@/store/index'
  ...
  import {eClearanceProcessing,eEarlyclearanceOperation} from '@/api/carLoadAfterApi'
    ...
    methods: {
      ...
      contractNoClick(data) {
       ...
       // sessionStorage.caseCheckList = JSON.stringify(tempArray);
        sessionStorage.eCSecondData = JSON.stringify(caseData);//合同号+流水号


				//跳转详情页面  路由传参数跳转详情
        pushUrl = "/earlyClearance/secondReview/pendingWork/pendingInfo";
      ...
      addDataList(){
        let oSubmitData = {
      	...
      	eEarlyclearanceOperation(oSubmitData).then(response => {
          if (response.data.code === "00000") {
            this.$message({
              message: '提交成功',
              type: 'success'
