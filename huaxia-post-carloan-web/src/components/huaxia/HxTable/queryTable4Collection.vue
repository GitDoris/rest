<template>
  <div class="padding">
    ...
    <div v-if="tableParam.isShoweds.isSearch">
      <queryTable :queryTables="tableParam.queryTables" :tableItems="tableParam" @search-click="btnClick" @reSet-click="btnClick"
            ...
            <template slot-scope="scope">
              ...
              <el-col v-else-if="item.code === 'operateColumn'" size="medium" align="center">
                <template v-for="btn in item.btns">
                  ...
                  <template v-else-if="btn.operateType === 'viewLogRemote'">
                    <el-popover
                      placement="left"
                      popper-class="showLog"
                      width="260"
                      trigger="click">
                      <div class="log" v-if="currLogs.length > 0" v-for="(l, i) in currLogs">
                        <div><span>登记人：</span>{{l.creator}}</div>
                        <div><span>登记时间：</span>{{l.createdTime}}</div>
                        <div><span>更新人：</span>{{l.updator}}</div>
                        <div><span>更新时间：</span>{{l.updatedTime}}</div>
                      </div>
                      <div class="log" v-else>
                        暂无日志
                      </div>
                      <el-button style="margin-right: 5px" slot="reference"
                                 @click="operateColumnClick(btn.operateType,scope.row)"
                                 :type="btn.type">{{btn.name}}
                      </el-button>
                    </el-popover>
                  </template>
                  ...
                  <template v-else-if="btn.operateType === 'invalid' || btn.operateType === 'allocate' || scope.row.validFlag === '有效'">
                    <el-button style="margin-right: 5px" :type="btn.type" v-show="scope.row.validFlag === '有效'"
