...
//免费软件，请注明出处
//
//安德获得麻省理工学院2012年版达斯汀·迪亚兹和雅各布·桑顿的授权
// http://ender.no.de/
//
//符合HTML5视频标签标准的视频字幕。
//只要把这个库添加到你的网页上，它就会扫描你的HTML5页面
//视频标签，如果它们包含一个<track> subtitle，它将加载并解析
//字幕文件(只有在SRT标准下)，并显示字幕
//播放视频。该库可以在一个页面中处理多个视频文件。
//目前，VideoSub将适用于所有的浏览器，即使它们具有本机跟踪功能
//支持，因为根本不需要支持. srt文件。
// ==============================================================================
...
(function (w) {
	...
	All.prototype = {
		videosub_timecode_min: function (tc) {
			const tcpair = tc.split(' --> ');
			return func.videosub_tcsecs(tcpair[0]);
		},

		videosub_timecode_max: function (tc) {
			const tcpair = tc.split(' --> ');
			return func.videosub_tcsecs(tcpair[1]);
		},

		videosub_tcsecs: function (tc) {
			const tc1 = tc.split(',');
			const tc2 = tc1[0].split(':');
			const secs = Math.floor(tc2[0] * 60 * 60) + Math.floor(tc2[1] * 60) + Math.floor(tc2[2]);
			return secs;
		},

		videosub_main: function () {
			...
			$VIDEOSUB('video').each(function (el) {
				...
				if (subtitlesrc.indexOf('.srt') != -1) { // we have a track tag and it's a .srt file
					...
					// subtitlesrc = "http://localhost:8080/DOD_106393189-1024x576-1769k.mp4.srt"
					// $VIDEOSUB.ajax({
					// 	method: 'get',
					// 	url: subtitlesrc,
					// 	type: 'html',
					...
					request.open('GET', subtitlesrc, true);
					request.onload = function (req) {
						if (request.status >= 200 && request.status < 400) {
							var data = request.responseText
							console.log("视频字幕：" + data);
					...
					$VIDEOSUB(el).addListener('seeked', function (an_event) {
						el.subcount = 0;
						while (func.videosub_timecode_max(el.subtitles[el.subcount][1]) < this.currentTime.toFixed(1)) {
							el.subcount++;
					...
					$VIDEOSUB(el).addListener('timeupdate', function (an_event) {
						...
						// console.log(this.currentTime.toFixed(1),func.videosub_timecode_min(el.subtitles[el.subcount][1]));
						// console.log(this.currentTime.toFixed(1),func.videosub_timecode_max(el.subtitles[el.subcount][1]));
						// console.log(el.subtitles);
						// console.log("------------------------------------------------------------------------------");
						// this.currentTime.toFixed(1) > func.videosub_timecode_min(el.subtitles[el.subcount][1])
						// this.currentTime.toFixed(1) < func.videosub_timecode_max(el.subtitles[el.subcount][1])
						if (this.currentTime.toFixed(1) > func.videosub_timecode_min(el.subtitles[el.subcount][1]) && this.currentTime.toFixed(1) < func.videosub_timecode_max(el.subtitles[el.subcount][1])) {
							subtitle = el.subtitles[el.subcount][2];
						}
						// is there a next timecode?
						if (this.currentTime.toFixed(1) > func.videosub_timecode_max(el.subtitles[el.subcount][1]) && el.subcount < (el.subtitles.length - 1)) {
							el.subcount++;
						}
						// update subtitle div	
