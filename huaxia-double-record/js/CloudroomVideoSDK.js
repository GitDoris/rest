...
if(window["000a0180686911e78dd0a45d36bb8c5c"] === undefined)
{
	...
	var CRVideo = function ()
		{
			var jsVersion = "3.11.18";
			var setSDKParamObj = {"RunInWebbrowser":1};
			var win = window,
				doc = document,
				nav = navigator,
				v = "",
			...
			return {"w3": w3cdom, "wk": webkit, "ie": ie,"ff":ff, "win": windows, "mac": mac,"v":v,"EnableLog2File":true,"EnableLogReport":true,"dblClickFullScreen":false,"jsVersion":jsVersion,"setSDKParamObj":setSDKParamObj};
		}()
    ...
    */
	 ...
	 * 设置工具条的高度
	 * @access public
	 * @param {number} size  -  工具条高度,取值范围(24-48) 
	 */
    CRVideo.MediaContainer.prototype.setToolBarSize = function(size)
	{
        this._handler.setToolBarSize(size);
	}
	 /**
	 * 设置全屏
	 * @access public
	 * @param {bool} bFullScreen  -  true 全屏
	 */
    CRVideo.MediaContainer.prototype.setFullScreen = function(bFullScreen)
	{
        this._handler.setFullScreen(bFullScreen);
	}
	/**
	 * 获取全屏
	 * @access public
	 * @return {bool} bFullScreen  -  true 全屏
	 */
    CRVideo.MediaContainer.prototype.getFullScreen = function()
	{
        return this._handler.getFullScreen();
	}
	 /**
	 * 绘制模式，是否拉伸绘制
	...
	/**
	 * 设置是否支持双击全屏
     * @param {bool} value - true: 支持双击全屏； false:不支持双击全屏
	 */
    CRVideo.MediaContainer.prototype.dblClickFullScreen = function(value)
	{
        if(value === undefined)
        {
            return this._handler.dblClickFullScreen;
        }else
        {
            this._handler.dblClickFullScreen = value;
        }
	}
     /**
	 * 工具条是否可用
	...
	/**
	 *叠加视频时，将视频容器放在上层
	 *注意：创建及布局UI控件时，底层的先创建先放入DOM树
	 * @access public
	 */
	CRVideo.VideoContainer.prototype.raise = function()
	{
		this._handler.raise();
	}
    /**
	...
	/**在视频窗口上显示一个遮罩图片
	 * @param {string} base64PNG - base64格式的png图像数据
	 * @param {bool} bScaleToFitVideoUI - true:铺满视频窗口，false:居中显示
	 * @access public
	 */
	CRVideo.VideoContainer.prototype.setMaskPic = function(base64PNG,bScaleToFitVideoUI)
	{
		this._handler.setMaskPic(base64PNG,bScaleToFitVideoUI);
	}
	/**设置拍照区域
	* @param {number} p_x - 相对于VideoUI控件的左上角位置
	 * @param {number} p_y - 相对于VideoUI控件的左上角位置
	 * @param {number} p_w - 拍照的图像大小
	 * @param {number} p_h - 拍照的图像大小
	 * @access public
	 */
	CRVideo.VideoContainer.prototype.setSavePicRect = function(p_x, p_y,p_w,p_h)
	{
		this._handler.setSavePicRect(p_x, p_y,p_w,p_h);
	}
	/**获取图像宽
	 * @access public
	 * @return {number} 宽
	 */
	CRVideo.VideoContainer.prototype.getPicWidth = function()
	{
		return this._handler.getPicWidth();
	}
	/**获取图像高
	 * @access public
	 * @return {number} 高
	 */
	CRVideo.VideoContainer.prototype.getPicHeight = function()
	{
		return this._handler.getPicHeight();
	}
	/**设置显示目标用户的第2、第3档视频
	 * @access public
	 * @param {string} ◦userID - 目标用户ID
	 * @param {string} ◦videoID - 目标摄像头ID
	 * @param {number} ◦qualityLv - 视频档位(1:默认档位，2:对应quality2，3:对应quality3，如果2,3未开启时将显示1档的视频流）
	 */
	CRVideo.VideoContainer.prototype.setVideo2 = function(usrID, videoID, qualityLv)
	{
		this._handler.setVideo2(usrID, videoID, qualityLv);
	}
	/**获取当前显示的视频档位
	 * @access public
	 * @return {number} 视频档位
	 */
	CRVideo.VideoContainer.prototype.getQualityLv = function()
	{
		return this._handler.getQualityLv();
	}
	 /**
	 * 设置全屏
	 * @access public
	 * @param {bool} bFullScreen  -  true 全屏
	 */
    CRVideo.VideoContainer.prototype.setFullScreen = function(bFullScreen)
	{
        this._handler.setFullScreen(bFullScreen);
	}
	/**
	 ...
	 * @return {bool} bFullScreen  -  true 全屏
	 */
    CRVideo.VideoContainer.prototype.getFullScreen = function()
	{
        return this._handler.getFullScreen();
	}
/**
	 ...
	 */
    CRVideo.VideoContainer.prototype.setToolbarHeight = function(height)
	{
        this._handler.setToolbarHeight(height);
	}
...
/**
	 ...
	 * @param
	 */
    CRVideo.VideoContainer.prototype.setToolButton = function(id, iconFileName,toolTip)
	{
        this._handler.setToolButton(id, iconFileName,toolTip);
	}
...
/**
	 ...
	 * @param 
	 */
    CRVideo.VideoContainer.prototype.lockNickNameText = function(lockNickNameText)
	{
        this._handler.lockNickNameText(lockNickNameText);
	}
	 /**
	 * 设置显示的视频画面是否保持比例
     * @param {bool} value - true: 保持比例不拉伸， false:不保持比例进行拉伸
     ...
     * @param {bool} value - true: 支持双击全屏； false:不支持双击全屏
	 */
    CRVideo.VideoContainer.prototype.dblClickFullScreen = function(value)
	{
        if(value === undefined)
        {
            return this._handler.dblClickFullScreen;
	 ...
	 * @access public
	 * @param {string} pathFileName - 本地绝对路径文件名(支持格式：bmp, png, gif, jpg, jpeg)
	 * @return {number} 0:成功； 非0：保存遇到的错误码；
	 */
	...
	CRVideo.ScreenShareContainer.prototype.setPenStyle = function(intRgba,penWidth)
	{
	   	...
	   	this._handler.setPenStyle(intRgba,penWidth);
	}
	/**
	*是否禁止出现浮动工具条
	 * @param {bool} bDisabled - true ：禁止出现浮动工具条; false：允许出现浮动工具条。默认出现。
	*/
	CRVideo.ScreenShareContainer.prototype.disableFloatToolbar = function(bDisabled)
	{
	    this._handler.disableFloatToolbar(bDisabled);
	}
	/**
	*获取远端屏幕的实际宽度
	* @return  远端屏幕的实际宽度（number）
	*/
	CRVideo.ScreenShareContainer.prototype.getPicWidth = function()
	{
	    return this._handler.getPicWidth();
	}
	/**
	*获取远端屏幕的实际高度
	* @return  远端屏幕的实际高度（number）
	*/
	CRVideo.ScreenShareContainer.prototype.getPicHeight = function()
	{
	    return this._handler.getPicHeight();
	}
	/**
	*鼠标是否在控件上
	* @return 鼠标是否在控件上，true- 在控件上； false - 不在控件上 {bool}
	*/
	CRVideo.ScreenShareContainer.prototype.isCursorInUI = function()
	{
	    return this._handler.isCursorInUI();
	}
	/**
	*鼠标在屏幕共享UI控件上映射到远端的坐标
	* @return json格式｛"x"=1,"y"=1}
	*/
	CRVideo.ScreenShareContainer.prototype.getCursorPosInRemote = function()
	{
	    return JSON.parse(this._handler.getCursorPosInRemote());
	}
	 /**
	 * 设置全屏
	 * @access public
	 * @param {bool} bFullScreen  -  true 全屏
	 */
    CRVideo.ScreenShareContainer.prototype.setFullScreen = function(bFullScreen)
	{
        this._handler.setFullScreen(bFullScreen);
	}
	/**
	 ...
	 * @return {bool} bFullScreen  -  true 全屏
	 */
    CRVideo.ScreenShareContainer.prototype.getFullScreen = function()
	{
        return this._handler.getFullScreen();
	}
	 /**
	 * 配置是否允许双击全屏
	 * @access public
	 * @param {bool} bVal  -  bVal: true:允许(缺省值)， false：不允许
	 */
    CRVideo.ScreenShareContainer.prototype.setDblClickFullScreen = function(bVal)
	{
        this._handler.setDblClickFullScreen(bVal);
	}
	/**
	 * 获取是否允许双击全屏
	 * @access public
	 * @return {bool} bVal  -  true 允许， false：不允许
	 */
    CRVideo.ScreenShareContainer.prototype.getDblClickFullScreen = function()
	{
        return this._handler.getDblClickFullScreen();
	}
	 /**
	 * 绘制模式，是否拉伸绘制
	 * @param {bool} value - true: 保持比例不拉伸， false:不保持比例进行拉伸
	 ...
	 * 白板的呈现容器
	 * @class
	 * @extends CRVideo.BoardContainer
	 */
	CRVideo.BoardContainer = function()
	{
		CRVideo._containerList.push(this)
	}
	CRVideo.BoardContainer.prototype = new CRVideo.Container()

	CRVideo.BoardContainer.prototype.__addCallBack = function()
	{
	 ...
	 *获取会议中所有白板ID的列表
	 * @access public
     * @return 白板ID字符串数组 {string[]}
	 */
	CRVideo.BoardContainer.prototype.getBoardIDs = function()
	{
		return (this._handler.getBoardIDs()).split(";");
	}
	/**
	 *获取指定白板的信息
	 * @access public
	 * @param {string} boardID - 白板ID
     * @return json格式的白板信息结构体 {BoardObj}
	 */
	CRVideo.BoardContainer.prototype.getBoardInfo = function(boardID)
	{
		return JSON.parse(this._handler.getBoardInfo(boardID));
	}
	/**
	 *获取当前白板ID
	 * @access public
     * @return 当前白板ID {string}
	 */
	CRVideo.BoardContainer.prototype.getCurBoardID = function()
	{
		return this._handler.getCurBoardID();
	}
	/**
	 *获取当前白板的页号
	 * @access public
     * @return 当前白板的页号 {number}
	 */
	CRVideo.BoardContainer.prototype.getCurPageNo = function()
	{
		return this._handler.getCurPageNo();
	}
	/**
	 *设置当前白板和页号
	 * @access public
     ...
     * @return 无返回值
	 */
	CRVideo.BoardContainer.prototype.setCurBoard = function(boardID,pageNo)
	{
		this._handler.setCurBoard(boardID,pageNo);
	}
	/**
	 *撤销当前操作
	 * @access public
     * @return 无返回值
	 */
	CRVideo.BoardContainer.prototype.undo = function()
	{
		this._handler.undo();
	}
	/**
     ...
     * @return 无返回值
	 */
	CRVideo.BoardContainer.prototype.redo = function()
	{
		this._handler.redo();
	}
	/**
     ...
     * @return 无返回值
	 */
	CRVideo.BoardContainer.prototype.clear = function()
	{
		this._handler.clear();
	}
	/**
	 *获取白板是否处于缩放模式
	 * @access public
     * @return {bool} 白板是否处于缩放模式；true - 缩放模式
	 */
	CRVideo.BoardContainer.prototype.isScaledContents = function()
	{
		return this._handler.isScaledContents();
	}
	/**
	 *设置白板的缩放模式
	 * @access public
	 * @param {bool} scaleVal - 白板的缩放模式值（true ：缩放模式）
     * @return 无返回值
	 */
	CRVideo.BoardContainer.prototype.setScaledContents = function(scaleVal)
	{
		this._handler.setScaledContents(scaleVal);
	}
	/**
	 *设置画笔颜色值
	 * @access public
	 * @param {PenColorObj} penColorObj - 画笔颜色值
     * @return 无返回值
	 */
	CRVideo.BoardContainer.prototype.setColor = function(penColorObj)
	{
		var rgb = parseInt(penColorObj.color.substring(1), 16);
		var a = parseInt(penColorObj.opacity*255);
		var color = a << 24 | rgb;
		this._handler.setColor(color);
	}
	/**
	 *获取画笔颜色值
	 * @access public
     * @return {PenColorObj} penColorObj - 画笔颜色值
	 */
	CRVideo.BoardContainer.prototype.getColor = function()
	{
		var penColorObj = {};

		var colorNum = this._handler.getColor();
		 if(colorNum < 0){
	        colorNum = -colorNum;
	        colorNum = colorNum-1;
	        var str = (Array(32).join("0")+colorNum.toString(2)).slice(-32);
	        str = exchange(str);
	   	 }else{
	        var str = (Array(32).join("0")+colorNum.toString(2)).slice(-32);
	    }
		...
		var hexStr = parseInt(str.substring(8), 2).toString(16);
		while(hexStr.length < 6){
			hexStr = "0" + hexStr;
		}
		penColorObj.color = "#" + hexStr;
		penColorObj.opacity = 0;
     ...
     * @return 无返回值
	 */
	CRVideo.BoardContainer.prototype.setLineWidth = function(width)
	{
		this._handler.setLineWidth(width);
	}

	/**
	 *获取画笔宽度
	 * @access public
     * @return {number} width - 画笔宽度
	 */
	CRVideo.BoardContainer.prototype.getLineWidth = function()
	{
		return this._handler.getLineWidth();
	}
	/**
	 *设置白板只读
	 * @access public
	 * @param {bool} bReadOnly - 设置白板只读
     * @return 无返回值
	 */
	CRVideo.BoardContainer.prototype.setReadOnly = function(bReadOnly)
	{
		this._handler.setReadOnly(bReadOnly);
	}

	/**
	 *获取白板是否只读
	 * @access public
     * @return {bool} width - 白板是否只读
	 */
	CRVideo.BoardContainer.prototype.isReadOnly = function()
	{
		return this._handler.isReadOnly();
	}
}
	/**
	 * 获取web js版本号
	 * @access public
	 * @returns {string} 返回云屋SDK js的版本号
	 */
	CRVideo_GetSDKJsVersion = function()
	{
		return CRVideo.jsVersion;
	}
...
if(window["000a0180686911e78dd0a45d36bb8c5c"] === undefined)
{
	...
	*/
	 ...
	 * @static
	 */
	CRVideo_CRVIDEOSDK_NOT_LOGIN = 12;			  
	//base64转换失败
	/**
	 * base64转换失败
	 * @static
	 */
	CRVideo_CRVIDEOSDK_BASE64_COV_ERR = 13;
	 ...
	 * @static
	 */
	CRVideo_VOICEENG_INITFAILED = 208;
	 ...
	 * @static
	 */
	CRVideo_MEETROOMLOCKED = 804;
	//余额不足
	/**
	 * 余额不足
	 * @static
	 */
	CRVideo_BALANCELESSERROR = 805;
	//业务权限未开启 
	/**
	 * 业务权限未开启 
	 * @static
	 */
	CRVideo_SEVICE_NOTENABLED = 806;
	//不能再次登录 
	/**
	 * 不能再次登录 
	 * @static
	 */
	CRVideo_ALREADYLOGIN = 807;
	//抓屏失败
	/**
	 * 抓屏失败 
	 * @static
	 */
	CRVideo_CATCH_SCREEN_ERR = 900;
	//单次录制达到最大时长(8h)
	/**
	 * 单次录制达到最大时长(8h) 
	 * @static
	 */
	CRVideo_RECORD_MAX = 901;
	//磁盘空间不够
	/**
	 * 磁盘空间不够 
	 * @static
	 */
	CRVideo_RECORD_NO_DISK = 902;
	//录像的分辨率超出限制 
	/**
	 * 录像的分辨率超出限制
	 * @static
	 */
	CRVideo_RECORD_SIZE_ERR = 903;
	/**
	 * 发送信令数据过大
	 * @static
	 */
	 ...
	 * @static
	 */ 
	CRVideo_CRVIDEOSDK_FILE_NOT_EXIST = 1400;
	/**
	 * 文件读失败
	 * @static
	 */ 
	CRVideo_CRVIDEOSDK_FILE_READ_ERR = 1401;
	/**
	 * 文件写失败
	 * @static
	 */ 
	CRVideo_CRVIDEOSDK_FILE_WRITE_ERR = 1402;
		//------------------------------------------
	//
	//web错误
	//
	 ...
	 * @static
	 */
	CRVideo_WEB_OCX_NOTINSTALLED = 2001;
	//不支持的浏览器
	/**
	 * 不支持的浏览器
	 * @static
	 */
	CRVideo_WEB_BROWER_NOTUPPORTED = 2002;
	//不支持的插件版本
	/**
	 * 不支持的插件版本
	 * @static
	 */
	CRVideo_OCX_VERSION_NOTUPPORTED = 2003;
...
if(window["000a0180686911e78dd0a45d36bb8c5c"] === undefined)
{
	...
	*/
	...
	/**
	 * 云端录制内容变化通知
	 * @callback CRVideo.CbProxy~CRVideo_SvrRecContentsChanged
	 */
	var CRVideo_SvrRecContentsChanged  = new CRVideo.CbProxy("CRVideo_SvrRecContentsChanged");
	 /**
	 * 开启屏幕共享的响应事件
	...
	var CRVideo_NotifyPlayPosSetted = new CRVideo.CbProxy("CRVideo_NotifyPlayPosSetted");
   	/**
	 * SDK通知文档转换进度
	 * @callback CRVideo.CbProxy~CRVideo_NotifyCoverStateChange
	 * @param {string} jsonState - 通知内容，json格式： {"state":0,"param":0}
	 */
	 var CRVideo_NotifyCoverStateChange = new CRVideo.CbProxy("CRVideo_NotifyCoverStateChange");
   	/**
	 * SDK通知文档转换失败
	 * @callback CRVideo.CbProxy~CRVideo_NotifyCoverFailed
	 * @param {number} coverErr - 失败原因：coverErr=0，代表传输失败;coverErr=1，代表转换失败
	 */
	 var CRVideo_NotifyCoverFailed = new CRVideo.CbProxy("CRVideo_NotifyCoverFailed");
   	/**
	 * SDK通知文档转换成功
	 * @callback CRVideo.CbProxy~CRVideo_NotifyCoverSuccess
	 * @param {string} 转换结果， json格式，定义见CoverRsltObjs
	 */
	 var CRVideo_NotifyCoverSuccess = new CRVideo.CbProxy("CRVideo_NotifyCoverSuccess");
	 /**
	 * SDK通知白板当前页号
	 * @callback CRVideo.CbProxy~CRVideo_NotifyBoardCurPageNo
	 * @param {string} boardID   - 当前的白板ID
	 * @param {number} pageNo - 当前白板的页号
	 * @param {number} pagePos1 - 扩展参数，当前白板页内X方向滚动条位置
	 * @param {number} pagePos2 - 扩展参数，当前白板页内Y方向滚动条位置
	 * @param {string} operatorID
	 */
	 var CRVideo_NotifyBoardCurPageNo = new CRVideo.CbProxy("CRVideo_NotifyBoardCurPageNo");
	 /**
	 * SDK通知请求分享
	 */
	 var CRVideo_NotifyRequestShare = new CRVideo.CbProxy("CRVideo_NotifyRequestShare");
	 /**
	 * SDK通知取消分享
	 */
	 var CRVideo_NotifyCancelShareRequestion = new CRVideo.CbProxy("CRVideo_NotifyCancelShareRequestion");
	/**
	 * SDK通知拒绝分享
	 */
	var CRVideo_NotifyRejectShareRequestion = new CRVideo.CbProxy("CRVideo_NotifyRejectShareRequestion");
	/**
	 * SDK通知查询文档列表结果 
	 * @param {string} dir   - 查询的目录
	 * @param {number} sdkErr - 0代表成功，非0参见CRVideo_ERR_DEF
	 * @param {CRVideo_ListDocFileRslt} rslt - 成功时的查询的结果，参见json格式之CRVideo_ListDocFileRslt
	 */
	var CRVideo_ListNetDiskDocFileRslt = new CRVideo.CbProxy("CRVideo_ListNetDiskDocFileRslt");
	/**
	 * SDK通知查询文档转换结果
	 * @param {string} svrPathFileName - 查询的文档名
	 * @param {number} sdkErr - 0代表成功，非0参见CRVideo_ERR_DEF
	 * @param {CRVideo_GetDocPageInfoRslt} rslt -成功时的查询的结果
	 */
	var CRVideo_GetNetDiskDocFilePageInfoRslt = new CRVideo.CbProxy("CRVideo_GetNetDiskDocFilePageInfoRslt");
	/**
	 * SDK通知文档删除结果
	 *  @param {string} svrPathFileName - 查询的文档名
	 *  @param {number} sdkErr - 0代表成功，非0参见CRVideo_ERR_DEF
	 */
	var CRVideo_DeleteNetDiskDocFileRslt = new CRVideo.CbProxy("CRVideo_DeleteNetDiskDocFileRslt");
	/**
	 * SDK通知文档传输进度
	 * @param {string}  svrPathFileName 传输的文档，或页文件名
 	 * @param {number} percent 传输进度（等于100代表传输完成）
	 */
	var CRVideo_NotifyNetDiskDocFileTrsfProgress = new CRVideo.CbProxy("CRVideo_NotifyNetDiskDocFileTrsfProgress");
	/**
	 * SDK通知云端录制文件状态变化
	 * * @callback CRVideo.CbProxy~CRVideo_NotifySvrRecFileState
	* @param {} json
	 */
	 var CRVideo_NotifySvrRecFileState = new CRVideo.CbProxy("CRVideo_NotifySvrRecFileState");
	 /**
	 * SDK通知本地录制、本地直播状态变化
	 * @callback CRVideo.CbProxy~CRVideo_LocMixerStateChanged
	 * @param {string} mixerID   - 混图器唯一标识
	* @param {number} state    - 态定义请参见 详见 CRVideo_MIXER_STATE
	 *  
	 */
	var CRVideo_LocMixerStateChanged = new CRVideo.CbProxy("CRVideo_LocMixerStateChanged");
	/**
     * @typedef {object} CRVideo_MixerOutputInfoObj -  录制文件、直播信息通知 
	 * @property {number} state   - 状态，请见后面描述 CRVideo_OUTPUT_STATE;
     * @property {number} duration  - 录录像文件时长，单位：毫秒。
	 * @property {number} fileSize    - 录像文件大小
	 * @property {number} errCode   - 错误码
     */
	 /**
	 * SDK通知本地录制文件、本地直播信息
	 ** @callback CRVideo.CbProxy~CRVideo_LocMixerOutputInfo
	 * @param {string} mixerID   - 混图器唯一标识
	 * @param {string} nameOrUrl - 录像名称、或直播url
	 * @param {string} outputInfo - json结构的字符串请参见CRVideo_MixerOutputInfoObj
	 */
	var CRVideo_LocMixerOutputInfo = new CRVideo.CbProxy("CRVideo_LocMixerOutputInfo");
 /**
	 * SDK通知本地录制文件、本地直播信息
	 ** @callback CRVideo.CbProxy~CRVideo_SvrMixerStateChanged
	 * @param {number} state 状态值请参考 CRVideo_MIXER_STATE
	 * @param {number} err - 错误值 
	 * @param {string} operatorID  - 引起变化的用户id
	 */
	var CRVideo_SvrMixerStateChanged = new CRVideo.CbProxy("CRVideo_SvrMixerStateChanged");
/**
	 * SDK通知云端录制、云端直播内容变化
	 ** @callback CRVideo.CbProxy~CRVideo_SvrMixerCfgChanged
	 */
	var CRVideo_SvrMixerCfgChanged = new CRVideo.CbProxy("CRVideo_SvrMixerCfgChanged");
	/**
	 * SDK通知云端录制文件、云端直播信息变化
	 ** @callback CRVideo.CbProxy~CRVideo_SvrMixerOutputInfo
	 * @param {string} outputInfo - json结构字符串，请参见CRVideo_MixerOutputInfoObj
	 */
	var CRVideo_SvrMixerOutputInfo = new CRVideo.CbProxy("CRVideo_SvrMixerOutputInfo");
	
	/**
	 * SDK通知设置会议成员昵称的结果 
	 ** @callback CRVideo.CbProxy~CRVideo_SetNickNameRsp
	 * @param {number} sdkErr -操作结果码, 0代表设置成功, 非0代表设置失败
	 * @param {string} userid - 用户id
	* @param {string} newname - 新的昵称 
	*/
	var CRVideo_SetNickNameRsp = new CRVideo.CbProxy("CRVideo_SetNickNameRsp");
	/**
	 * SDK通知某用户改变了昵称(改昵称的用户自身不会接收到此通知)
	 ** @callback CRVideo.CbProxy~CRVideo_NotifyNickNameChanged
	 * @param {string} userID - 改变昵称的某用户id
	 * @param {string} oldname - 改变昵称的某用户昵称(改变之前的昵称)
	 * @param {string} newname - 改变昵称的某用户昵称(改变之后的昵称)
	 */
	var CRVideo_NotifyNickNameChanged = new CRVideo.CbProxy("CRVideo_NotifyNickNameChanged");
	/**
	 * SDK通知视频分屏模式切换2
	 * @callback CRVideo.CbProxy~CRVideo_NotifyVideoWallMode2
	 * @param {CRVideo_VIDEOLAYOUTMODE} model  - 分屏模式
	 */
	var CRVideo_NotifyVideoWallMode2 = new CRVideo.CbProxy("CRVideo_NotifyVideoWallMode2");	
	/**
	* 第3方呼叫操作结果
	...
	* @param {number} sdkErr - 操作结果代码,定义见cr/error
	...
	var CRVideo_NotifyShareRectChanged = new CRVideo.CbProxy("CRVideo_NotifyShareRectChanged");
	 /**
	 * SDK通知白板中是否可以使用撤销功能
	 * @callback CRVideo.CbProxy~CRVideo_UndoEnableChange
	 * @param {bool} 是否可以使用撤销功能，为true才可以使用
	 */
	 var CRVideo_UndoEnableChange = new CRVideo.CbProxy("CRVideo_UndoEnableChange");
	 /**
	 * SDK通知白板中是否可以使用恢复功能
	 * @callback CRVideo.CbProxy~CRVideo_RedoEnableChange
	 * @param {bool} 是否可以使用恢复功能，为true才可以使用
	 */
	 var CRVideo_RedoEnableChange = new CRVideo.CbProxy("CRVideo_RedoEnableChange");
}
if(window["000a0180686911e78dd0a45d36bb8c5c"] === undefined)
{
	//---------------------------------------
	...
	CRVideo_Init2 = function(sdkUsePath,statCallSer,statMediaSer,statHttp){
		...
		{
			if(CRVideo.setSDKParamObj["NoCall"] === undefined){
				CRVideo.setSDKParamObj["NoCall"] = 0;
			}else{
				//nothing
			}		
		}else{
			statCallSer == 0 ? (CRVideo.setSDKParamObj["NoCall"] = 1):(CRVideo.setSDKParamObj["NoCall"] = 0);	
		}
		if(statMediaSer === undefined)
		{
			if(CRVideo.setSDKParamObj["NoMediaDatToSvr"] === undefined){
				CRVideo.setSDKParamObj["NoMediaDatToSvr"] = 0;
			}else{
				//nothing
			}		
		}else{
			statMediaSer == 0 ? (CRVideo.setSDKParamObj["NoMediaDatToSvr"] = 1):(CRVideo.setSDKParamObj["NoMediaDatToSvr"] = 0);	
		}
	...
	CRVideo_Init = function(oemID,sdkUsePath,statCallSer,statMediaSer,statHttp)
	{
		...
		try
		{
			...
			if(!CRVideo.ie)
			{
				...
				CRVideo._crBoard = document.getElementById("crBoardObjG");
			...
			}else
			{
                ...
                CRVideo._crBoard = document.getElementById("crBoardObj");
			...
			if(!CRVideo.ie)
			{
				...
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'svrRecContentsChanged',CRVideo_SvrRecContentsChanged);
				...
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'notifyCoverStateChange',CRVideo_NotifyCoverStateChange);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'notifyCoverFailed',CRVideo_NotifyCoverFailed);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'notifyCoverSuccess',CRVideo_NotifyCoverSuccess);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'notifyBoardCurPageNo',CRVideo_NotifyBoardCurPageNo);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'notifyRequestShare',CRVideo_NotifyRequestShare);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'notifyCancelShareRequestion',CRVideo_NotifyCancelShareRequestion);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'notifyRejectShareRequestion',CRVideo_NotifyRejectShareRequestion);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'listNetDiskDocFileRslt',CRVideo_ListNetDiskDocFileRslt);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'getNetDiskDocFilePageInfoRslt',CRVideo_GetNetDiskDocFilePageInfoRslt);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'deleteNetDiskDocFileRslt',CRVideo_DeleteNetDiskDocFileRslt);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'notifyNetDiskDocFileTrsfProgress',CRVideo_NotifyNetDiskDocFileTrsfProgress);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'notifySvrRecFileState',CRVideo_NotifySvrRecFileState);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'locMixerStateChanged',CRVideo_LocMixerStateChanged);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'locMixerOutputInfo',CRVideo_LocMixerOutputInfo);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'svrMixerStateChanged',CRVideo_SvrMixerStateChanged);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'svrMixerCfgChanged',CRVideo_SvrMixerCfgChanged);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'svrMixerOutputInfo',CRVideo_SvrMixerOutputInfo);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'setNickNameRsp',CRVideo_SetNickNameRsp);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'notifyNickNameChanged',CRVideo_NotifyNickNameChanged);
				__crVideo_RegisterCallBack(CRVideo._crMeet, 'notifyVideoWallMode2',CRVideo_NotifyVideoWallMode2);
				...
				//Board管理接口
				//-----------------------------
				__crVideo_RegisterCallBack(CRVideo._crBoard, 'undoEnableChange',CRVideo_UndoEnableChange);
				__crVideo_RegisterCallBack(CRVideo._crBoard, 'redoEnableChange',CRVideo_RedoEnableChange);


			}

			// var setSDKParams = {"RunInWebbrowser":1, "NoCall":0,"NoMediaDatToSvr":0}
			// if(statCallSer == 0)
			// {
			// 	setSDKParams["NoCall"] = 1
			// }
			// if(statMediaSer == 0)
			// {
			// 	setSDKParams["NoMediaDatToSvr"] = 1
			// }
			CRVideo._crSdk.setSDKParams(JSON.stringify(CRVideo.setSDKParamObj));
			CRVideo._crSdk.enableLogReport(CRVideo.EnableLogReport);
			CRVideo._crSdk.enableLog2File(CRVideo.EnableLog2File);
			if(oemID == ""){
	...
	{
		CRVideo.EnableLog2File = bEnable;
	}
	 /**
	 *  设置是否开启日志上报
	 * @access public SDK中日志上报默认是打开的,这个接口在init之前可调用
	 * @param {number} bEnable - 1表示开启上报，0表示不开启上报
	 */
	CRVideo_EnableLogReport = function(bEnable)
	{
		CRVideo.EnableLogReport = bEnable;
	}
	/**
	 *  设置SDK参数 
	 * @access public 这个接口在init之前调用有效
	 * @param {object} jsonVal - jsonVal json格式, 如：{ "NoCall" : 1 }
	 */
	CRVideo_SetSDKParams = function(jsonVal)
	{
		//1.标准json格式参数 2.有默认字段则改默认字段中的值 3，新字段，加入到默认obj中
		if(typeof jsonVal == 'object' && jsonVal ){
			var jsonStr = JSON.stringify(jsonVal);
			var json = JSON.parse(jsonStr);

			for(var key in json){
				CRVideo.setSDKParamObj[key] = json[key]; //change or add
			}
		}
	}
...
/**
	 * 文件下载的流量控制
	 * @access public
	 * @param {number} maxBytePerSec - 每秒下载的最大字节数。-1代表不进行流控
	 * @returns 无
	 */
	CRVideo_SetFileDownloadRate = function(maxBytePerSec)
	{
		CRVideo._crSdk.setFileDownloadRate(maxBytePerSec);
	}
/**
	 * 检查插件是否运行在ie的保护模式下
	 * @access public
	 * @returns {bool} true - ie保护模式下； false - 非ie或非保护模式
	 */
	CRVideo_IsIEProtectedMode = function()
	{
		return CRVideo._crSdk.isIEProtectedMode();
	}
/**
	 * 获取ie的保护模式下可读写的路径
	 * @access public
	 * @returns {string} 路径
	 */
	CRVideo_GetWriteablePathInIEProtectedMode = function()
	{
		return CRVideo._crSdk.getWriteablePathInIEProtectedMode();
	}
/**
	 * 
	 * @access public
	 ...
	 * @returns 无
	 */
	CRVideo_SetAliyunOssAccountInfo = function(accessKey,accessSecret)
	{
		CRVideo._crSdk.setAliyunOssAccountInfo(accessKey,accessSecret);
	}
	/**
	 * 获取Plugin版本号
	 * @access public
	 ...
	 * 往日志文件中写入内容
	 * @access public
	 * @param {number} level - 0:SDKLEVEL_DEBUG; 1:SDKLEVEL_INFO; 2:SDKLEVEL_WARN; 3:SDKLEVEL_ERR; 4:SDKLEVEL_CRIT
	 * @param {string} log - 日志内容
	 * @returns undefined
	 */
	CRVideo_WriteLog = function(level, log)
	{
		return CRVideo._crSdk.writeLog(level, log);
	}
	 ...
	 * @param {number} needPswd - 是否创建会议密码（=0时：会议无密码，>0时：密码由系统自动生成）
	 * @param {string} cookie - 自定义数据 (在回调时，回传给调用者)
	 */
	CRVideo_CreateMeeting = function(meetSubject,needPswd,cookie)
	{
		if(needPswd === undefined)
		{
			needPswd = 0;
		}
		if(cookie === undefined)
		...
		CRVideo._crMgr.createMeeting(meetSubject,needPswd,cookie);
	}
	/**
	 * 销毁视频会议
	 * 回调CRVideo_DestroyMeetingRslt
	 * @access public
	 * @param {number} meetID - 会议id
	 * @param {string} cookie - 自定义数据(在响应消息中回传给调用者)，不需要时传空字符串
	 */
	CRVideo_DestroyMeeting = function(meetID,cookie)
	{
		...
		CRVideo._crMgr.destroyMeeting(meetID,cookie);
	}
	/**
	 * 发起呼叫，邀请用户参加视频会话。
	 ...
	 * @returns {string} 分配的任务ID
	 */
	CRVideo_SendFile = function(targetUserId,fileName)
	{
		return CRVideo._crMgr.sendFile(targetUserId,fileName);
	}
	/**
	 * 取消数据发送
	 ...
	 * @param {num} msecond - 视频播放的位置(毫秒)
	 * @param {string} markText - 打点信息
	 */
	 CRVideo_SetMarkText = function(videoFilePathName,msecond,markText)
	{
		if(markText == undefined){
			markText = "";
		}
		 CRVideo._crMgr.setMarkText(videoFilePathName,msecond,markText);
	}
	/**
	 * 移除视频文件某处打点信息
	 * @access public
	 ...
	 * @param {num} msecond - 视频播放的位置(毫秒)
	 */
	 CRVideo_RemoveMarkText = function(videoFilePathName,msecond)
	{
		 CRVideo._crMgr.removeMarkText(videoFilePathName,msecond);
	}
	/**
	 * 获取所有的视频打点信息
	 * @access public
	 ...
	 * 尝试读取磁盘文件
	 * @access public
	 * @param {string} diskPathFileName  磁盘文件路径名
	 * @returns {number} sdkErr
	 */
	 CRVideo_TryReadFile = function(diskPathFileName)
	{
		return CRVideo._crMgr.tryReadFile(diskPathFileName);
	}
	/**
	 * 读取磁盘文件
	 * @access public
	 * @param {string} diskPathFileName  磁盘文件路径名
	 * @returns {string} base64编码格式的文件数据
	 */
	 CRVideo_ReadFileAsBase64 = function(diskPathFileName)
	{
		return CRVideo._crMgr.readFileAsBase64(diskPathFileName);
	}
	/**
	 * 写文件到指定的路径
	 * @access public
	 * @param {string} diskPathFileName  目的文件路径名
	 * @param {string} base64Dat  base64编码格式的文件数据
	 * @returns {number} sdkErr
	 */
	 CRVideo_WriteBase64ToFile = function(diskPathFileName, base64Dat)
	{
		return CRVideo._crMgr.writeBase64ToFile(diskPathFileName, base64Dat);
	}
	/**
	 * 获取用户打开的全路径文件名
	 * @access public
	 * @param  {string}  title 打开文件框的标题
   	 * @param  {string}  directory 打开对话框初始路径， 不设初始值时请传空
   	 * @param  {string}  filter 文件类型过滤器（可为空），示例：Images (*.png *.xpm *.jpg);;Text files (*.txt)
	 * @returns {string} 空串表示用户取消， 否则返回用户选择的文件路径文件名
	 */
	 CRVideo_GetOpenFileName = function(title,directory, filter)
	{
		return CRVideo._crMgr.getOpenFileName(title,directory, filter);
	}
	/**
	 * 获取用户保存的全路径文件名
	 * @access public
	 * @param  {string}  title 打开文件框的标题
   	 * @param  {string}  directory 打开对话框初始路径， 不设初始值时请传空
   	 * @param  {string}  filter 文件类型过滤器（可为空），示例：Images (*.png *.xpm *.jpg);;Text files (*.txt)
	 * @returns {string} 空串表示用户取消， 否则返回用户保存的文件路径文件名
	 */
	CRVideo_GetSaveFileName = function(title,directory, filter)
	{
		return CRVideo._crMgr.getSaveFileName(title,directory, filter);
	}
	/**
	 * @typedef {object} CRVideo_GetMeetingsParam -  获取会议列表的参数,json结构体
	 * @property {number} QueryMode - 0:查询全部， 1:查询我创建的会议
	 */
	/**
	 * 获取视频会议列表 
	 * @access public
	 * @param  {CRVideo_GetMeetingsParam}  json  -  获取视频会议列表的参数,json结构体
   	 * @param  {string}  cookie  -  自定义数据(在响应消息中回传给调用者)，不需要时传空字符串
	 * @returns 无
	 */
	CRVideo_GetMeetings2 = function(json,cookie)
	{
		CRVideo._crMgr.getMeetings2(JSON.stringify(json),cookie);
	}
	/**
	 * @typedef {object} CRVideo_CreatInfoObj -  - 创建会议信息,json结构体
	 * @property {string} meetSubject - 会议名称
	 * @property {number} needPswd -  是否创建密码（0:会议无密码，1:系统为会议室创建一个密码）
	 * @property {number} isRegular - 是否固定会议；（0:普通会议，会议室无人30分钟后，自动消毁; 1:固定会议，会议室一直存在直到用户主动消毁它）
	 */
	/**
	 * 创建视频会议 
	 * @access public
	 * @param  {CRVideo_CreatInfoObj}  json  - 创建会议信息，json结构体定义请参见creatInfoObj
   	 * @param  {string}  cookie  -  自定义数据(在响应消息中回传给调用者)，不需要时传空字符串
	 * @returns 无
	 */
	CRVideo_CreateMeeting2 = function(json,cookie)
	{
		CRVideo._crMgr.createMeeting2(JSON.stringify(json),cookie);
	}
	/**
	 * 获取企业下指定用户的在线状态（包括呼叫会议状态、免打扰状态）
	 * @access public
	 * @param  {string}   usrID  - 
   	 * @param  {string}  cookie  -  自定义数据(在响应消息中回传给调用者)，不需要时传空字符串
	 * @returns 无
	 */
	CRVideo_GetOneUserStatus = function(usrID,cookie)
	{
		CRVideo._crMgr.getOneUserStatus(usrID,cookie);
	}
/**
	 * 移动文件
	 * @access public
	 * @param  {string}   srcDiskFileName  - 
   	 * @param  {string}  dstDiskFileName  -  
	 * @returns {number} 错误码
	 */
	CRVideo_MoveFile = function(srcDiskFileName,dstDiskFileName)
	{
		return CRVideo._crMgr.moveFile(srcDiskFileName,dstDiskFileName);
	}
	 ...
	 * @returns {bool} 返回值 true false
	 */
	CRVideo_SetVideoCfg = function(cfg)
	{
		return CRVideo._crMeet.setVideoCfg(JSON.stringify(cfg));
	}
	/**
	 * 开始获取语音pcm数据
	 * @access public
	 ...
	 * 返回值 sdkErr 操作结果码, 0代成功, 非0代表入会失败，取值参考CRVideo_ERR_DEF
	 */
	 ...
	 * 获取录制的文件时长（以毫秒为单位）
	 * @access public
	 * @returns {number} 返回录制的文件时长（以毫秒为单位）
	 */
	CRVideo_GetRecDurationMs = function()
	{

		return CRVideo._crMeet.getRecDurationMs();
	...
	{
		return CRVideo._crMeet.getSvrRecordInfo()
	}
	/* 获取当前云端录制的状态
	 * @access public
	...
	{
		return CRVideo._crMeet.getSvrRecordState()
	}
	 ...
	 * @param {string} filename  - 媒体文件名（本地全路径文件名，或网络流媒体，支持的协议有：http, rtmp, rtsp)
	 ...
	 * @return {bool}
	 */
	CRVideo_IsCovering = function()
	{
		return CRVideo._crMeet.isCovering();
	}
	/**
	 * 转换文档
	 * @access public
	 * @param {string} srcFilePathName - 本地需要转换的文件（包含路径）
	 * @param {string} jsonParam - 保留字段
	 */
	CRVideo_CoverFile = function(srcFilePathName, jsonParam)
	{
		CRVideo._crMeet.coverFile(srcFilePathName, jsonParam);
	}
	/**
	 * 取消转换
	 * @access public
	 */
	CRVideo_CancelCover = function()
	{
		CRVideo._crMeet.cancelCover();
	}
	/**
	 * @typedef {object} CRVideo_VideoAttributesObj - 视频设备私有属性 
	 * @property {number} disabled  - 取值0:不禁用此设备(默认值)，1：禁用此设备；
	 * @property {string} size  - 参见VideoCfgObj的描述，未配置时采用全局配置VideoCfgObj
	 * @property {number} fps  - 参见VideoCfgObj的描述，未配置时采用全局配置VideoCfgObj
	 * @property {number} maxbps   - 参见VideoCfgObj的描述，未配置时采用全局配置VideoCfgObj
	 * @property {number} qp_min    - 参见VideoCfgObj的描述，未配置时采用全局配置VideoCfgOb
	 * @property {number} qp_max   - 参见VideoCfgObj的描述，未配置时采用全局配置VideoCfgOb
	 * @property {number} qp_max   - 第二档质量配置，支持的属性有：size, maxbps, qp_min, qp_max
	 * @property {number} qp_max   - 第三档质量配置，支持的属性有：size, maxbps, qp_min, qp_max
	 */
	/**
	 * 设置某个摄像头私有参数（未设置的参数，使用全局参数）
	 * @access public
	 * @param {string} videoID - 本地摄像头id
	 * @param {CRVideo_VideoAttributesObj} jsonAttributes - json格式，支持的参数有：
{"disabled": 取值0/1}，1:代表不启用此摄像头；
	 */
	CRVideo_SetLocVideoAttributes = function(videoID, jsonAttributes)
	{
		CRVideo._crMeet.setLocVideoAttributes(videoID, JSON.stringify(jsonAttributes));
	}
	/**
	 * 获取某个摄像头私有参数
	 * @access public
	 * @param {string} videoID - 本地摄像头id
     * @return {CRVideo_VideoAttributesObj} - 私有参数, json格式
	 */
	 CRVideo_GetLocVideoAttributes = function(videoID)
	{
		return JSON.parse(CRVideo._crMeet.getLocVideoAttributes(videoID));
	}
	/**
	 * 设置白板当前页号
	 * @access public
	 * @param {string} boardID - 当前白板ID
	 * @param {number} pageNo - 当前白板的当前页号
	 * @param {number} pagePos1 - 扩展参数1（设置页内X方向滚动条的滚动位置）
	 * @param {number} pagePos2 - 扩展参数2（设置页内Y方向滚动条的滚动位置）
	 */
	 CRVideo_SetBoardCurPageNo = function(boardID,pageNo,pagePos1,pagePos2)
	{
		CRVideo._crMeet.setBoardCurPageNo(boardID,pageNo,pagePos1,pagePos2);
	}
	/**
	 *创建自定义摄像头（sdk最大支持5个).添加成功后与本地摄像头处理一致；getAllVideoInfo接口可以识别摄像头类型。
	 * @access public
	 * @param {string} camName - 摄像头名称
	 * @param {number} pixFmt - 参见VIDEO_FORMAT定义
	 * @param {number} width - 图像宽度
	 * @param {number} height - 图像高度
	 * @param {string} extParams - 扩展参数，没有为空
	 * @return {number} -  <0 ,失败, >=0, 为videoID(摄像头ID)
	 */
	 CRVideo_CreateCustomVideoDev = function(camName,pixFmt,width,height,extParams)
	{
		return CRVideo._crMeet.createCustomVideoDev(camName,pixFmt,width,height,extParams);
	}
	/**消毁自定义摄像头
	 * @access public
	 * @param {number} videoID - 摄像头id
	 */
	 CRVideo_DestroyCustomVideoDev = function(videoID)
	{
		CRVideo._crMeet.destroyCustomVideoDev(videoID);
	}
	/**
	 * 输入摄像头图像数据
	 * @access public
	 * @param {number} videoID - 摄像头id
	 * @param {string} base64Dat -  图像数据，base64编码。（请保证格式、和尺寸与摄像头匹配）
	 * @param {number} timeStamp - 保留字段，请填0，
	 */
	 CRVideo_InputCustomVideoDat = function(videoID,base64Dat,timeStamp)
	{
		CRVideo._crMeet.inputCustomVideoDat(videoID,base64Dat,timeStamp);
	}
	/**
	 * 输入摄像头图像数据2
	 * @access public
	 ...
	 * @param {number} timeStamp - 保留字段，请填0，
	 */
	CRVideo_InputCustomVideoDat2 = function(videoID,base64Dat,timeStamp)
	{
		CRVideo._crMeet.inputCustomVideoDat2(videoID,base64Dat,timeStamp);
	}
	/**
	 *  请求某人开启屏幕共享
	 * @access public
	 * @param {number} sharerId - 需要开启共享的用户ID
	 * @param {string} p_param -  自定义参数，可传空
	 */
	CRVideo_RequestShare = function(sharerId,p_param)
	{
		CRVideo._crMeet.requestShare(sharerId,p_param);
	}
	/**
	 *取消屏幕共享请求
	 * @access public
	 * @param {number} sharerId -需要取消的共享者ID,与CRVideo_RequestShare的参数对应
	 */
	CRVideo_CancelShareRequestion = function(sharerId)
	{
		CRVideo._crMeet.cancelShareRequestion(sharerId);
	}
	/**
	 *拒绝分享请求
	 * @access public
	 * @param {number} requesterId - 请求者ID
	 * @param {string} param - 自定义参数，可传空
	 */
	CRVideo_RejectShareRequestion = function(requesterId,param)
	{
		CRVideo._crMeet.rejectShareRequestion(requesterId,param);
	}
	/**
	 * 创建桌面摄像头(sdk最大支持5个)
	 * @access public
	 * @param {string} camName - 摄像头名称
	 * @param {number} monitor - 显示器序号，与系统显示编号一致，-1代表主屏。(创建时对应monitorID不存在时将失败， 运行中monitorID对应的屏关闭时显示全黑，开启后恢复正常工作）
	 * @return {number}  - <0,  失败,  >=0, 为videoID(摄像头ID)
	 */
	CRVideo_CreateScreenCamDev = function(camName,monitor)
	{
		return CRVideo._crMeet.createScreenCamDev(camName,monitor);
	}
	/**
	 * 更新桌面摄像头
	 * @access public
	 * @param {number} videoID - 摄像头id
	 * @param {number} monitor -  显示器序号，与系统显示编号一致，-1代表主屏。(monitorID不存在时将失败）
	 * @return {bool}  - false失败, true成功
	 */
	CRVideo_UpdateScreenCamDev = function(videoID,monitor)
	{
		return CRVideo._crMeet.updateScreenCamDev(videoID,monitor);
	}
	/**
	 *  消毁桌面摄像头
	 * @access public
	 * @param {number} videoID - 摄像头id
	  */
	CRVideo_DestroyScreenCamDev = function(videoID)
	{
		CRVideo._crMeet.destroyScreenCamDev(videoID);
	}
	 ...
	 * @access public
	 * @param {string} dir - 要查询的目录，空或"/"时，代表查询根目录下的内容；
	  */
	 CRVideo_ListNetDiskDocFile = function(dir)
	 {
		 CRVideo._crMeet.listNetDiskDocFile(dir);
	 }
	/**
	 *  上传并转换文档
	 * @access public
	 * @param {string} svrPathFileName - 服务器的路径文件名，包含目录时，服务器自动创建对应目录
	  * @param {string} locPathFileName - 要上传文件的本地的路径文件名
	  */ 
	 CRVideo_UploadDocFileToNetDisk = function(svrPathFileName,locPathFileName)
	 {
		CRVideo._crMeet.uploadDocFileToNetDisk(svrPathFileName,locPathFileName);
	 }
	 /**
	 *  下载源始文档 
	 * @access public
	 * @param {string} svrPathFileName - 服务器的路径文件名，包含目录时，服务器自动创建对应目录
	  * @param {string} locPathFileName - 要上传文件的本地的路径文件名
	  */ 
	 CRVideo_DownloadNetDiskDocFile = function(svrPathFileName,locPathFileName)
	 {
		CRVideo._crMeet.downloadNetDiskDocFile(svrPathFileName,locPathFileName);
	 }
	/**
	 *  取消文档传输
	 * @access public
	 * @param {string} svrPathFileName - 服务器的路径文件名
	  */ 
	 CRVideo_CancelTransforNetDiskDocFile = function(svrPathFileName)
	 {
		CRVideo._crMeet.cancelTransforNetDiskDocFile(svrPathFileName);
	 }
	 /**
	 *  删除文档
	 * @access public
	 * @param {string} svrPathFileName - 服务器的路径文件名
	  */ 
	 CRVideo_DeleteNetDiskDocFile = function(svrPathFileName)
	 {
		CRVideo._crMeet.deleteNetDiskDocFile(svrPathFileName);
	 }
	 /**
	 *   获取文档的转换信息
	 * @access public
	 * @param {string} svrPathFileName - 服务器的路径文件名
	  */ 
	 CRVideo_GetNetDiskDocFilePageInfo = function(svrPathFileName)
	 {
		CRVideo._crMeet.getNetDiskDocFilePageInfo(svrPathFileName);
	 }
	 /**
	 *   下载文档转换后的页文件
	 * @access public
	 * @param {string} pagePathFileName - 页文件在服务器的路径文件名
	 * @param {string} locPathFileName - 文件下载后存放的本地的路径文件名，目录不存在时自动创建
	  */ 
	 CRVideo_DownloadNetDiskDocFilePage = function(pagePathFileName ,locPathFileName)
	 {
		CRVideo._crMeet.downloadNetDiskDocFilePage(pagePathFileName ,locPathFileName);
	 }
    /**
	 ...
	 * @access public
	 * @param {number} videoID  - 摄像头id
	 * @param {number} hParenWnd  - 高级配置界面的父窗口, 没有特别需求传0即可
	 * @returns {bool} 返回 true/false
	 * 
	 */
	CRVideo_ShowVideoAdvCfgDlg = function(videoID, hParenWnd)
	{
	    return CRVideo._crMeet.showVideoAdvCfgDlg(videoID, hParenWnd);
	}
	/**
     * @typedef {object} CRVideo_MixerCfgObj -  获混图器规格配置的参数,json结构体
     * @property {number} width - 图像宽度
	 *  @property {number} height  - 图像高度
	 * @property {number} frameRate  - 图像帧率，取值范围:1-30(值越大,cpu要求更高，录像推荐15帧，直播推存25帧)
	 *@property {number} bitRate  - 录制视频文件的最高码率，当图像变化小时，实际码率会低于此值。
	 *@property {number} defaultQP  - 录制视频文件的缺省质量，缺省值：26。取值范围：0~51，0表示完全无损, 51表示质量非常差，推荐高质量取值18，中质量26， 低质量34。
	 *@property {number} gop  - I帧周期(I帧越少码率越小，但直播延时会越大）； 文件录制建议15秒一个I帧取值：fpsx15； 直播建议4秒一个I帧取值: fpsx4;
     */
	/**
     * @typedef {[{},{}]} CRVideo_MutiMixerCfgsObj -  混图器规格配置列表,json结构体数组。[
{"id":"1", "cfg":{"width":640,"height":320,"frameRate":8,"bitRate":500000,"defaultQP":28,"gop":120}}]
     * @property {number} id  - 混图器编号
	 * @property {CRVideo_MixerCfgObj} cfg  - CRVideo_MixerCfgObj
     */
	/**
     * @typedef {object} CRVideo_MixerContentObj -  获混图器内容配置的参数,json结构体数组
     * @property {number} left - 在混图画面中的区域（相对于混图器尺寸）
	 * * @property {number} top - 在混图画面中的区域（相对于混图器尺寸）
	 * * @property {number} width - 在混图画面中的区域（相对于混图器尺寸）
	 * * @property {number} height - 在混图画面中的区域（相对于混图器尺寸）
	 * @property {number} type  - 类型 CRVideo_MIXER_VCONTENT_TYPE
	 * @property {number} keepAspectRatio - 内容保持原始比例，0不保持，1保持
	 * @property {{}}} param - ":{"camid":"usr2.1"}}
     */
	/**
     * @typedef {object} CRVideo_MutiMixerContentsObj -  获混图器内容配置的参数,json结构体数组
     * @property  {number} id  - 混图器编号
	 * @property {[{},{}]} content 混图器内容配置，参见CRVideo_MixerContentObj
     */
	/**
	 * 创建本地混图器
	 * @access public
	 * @param {string} mixerID  - 混图器唯一标识
	 * @param {CRVideo_MixerCfgObj} mixerCfg  - 混图器规格配置，json格式,详见CRVideo_MixerCfgObj
	 * @param {CRVideo_MixerContentObj} mixerContent  - 混图器内容配置，json格式,详见CRVideo_MixerContentObj
	 * @returns {number} 返回 错误码
	 * 
	 */
	CRVideo_CreateLocMixer = function(mixerID, mixerCfg, mixerContent)
	{
	    return CRVideo._crMeet.createLocMixer(mixerID, JSON.stringify(mixerCfg), JSON.stringify(mixerContent));
	}
	/**
	 * 更新本地混图器内容
	 * @access public
	 * @param {string} mixerID  - 混图器唯一标识
	 * @param {CRVideo_MixerContentObj} mixerContent  - 混图器内容配置，json格式的字符串,详见CRVideo_MixerContentObj
	 * @returns {number} 返回 错误码
	 * 
	 */
	CRVideo_UpdateLocMixerContent = function(mixerID,mixerContent)
	{
	    return CRVideo._crMeet.updateLocMixerContent(mixerID,JSON.stringify(mixerContent));
	}
	/**
	 * 消毁本地混图器
	 * @access public
	 ...
	 * 
	 */
	CRVideo_DestroyLocMixer = function(mixerID)
	{
	    CRVideo._crMeet.destroyLocMixer(mixerID);
	}
	/**
	 * 获取本地混图器状态 
	 * @access public
	 * @param {string} mixerID  - 混图器唯一标识
	 * @returns {number}  详见 CRVideo_MIXER_STATE
	 * 
	 */
	CRVideo_GetLocMixerState = function(mixerID)
	{
	    return CRVideo._crMeet.getLocMixerState(mixerID);
	}
	/**
     * @typedef {object} CRVideo_MixerOutputObj -  本地输出对象,json结构体数组
	 * @property {string} type  - 输出类型 CRVideo_MIXER_OUTPUT_TYPE
     * @property {string} filename - 录像路径文件名（本地录像名格式如：d:\1.mp4，服务器录像名格式如：/2018-11-21/1.mp4），支持的文件格式为mp4/ts/flv/avi，其中flv和ts两种格式在程序异常结束时产生的录制文件仍可用。
	 * @property {number} encryptType   - 录像文件是否加密，0:不加密，1:加密；
	 * @property {number} isUploadOnRecording  - 录像文件是否边录边传，0:不上传，1:边录边传; (此参数仅本地录像有效） 
	 * @property {string} serverPathFileName - 边录边传时，上传到服务器的路径文件名； (此参数仅本地录像有效）
	 * @property {string} liveUrl  - 直播推流地址，支持rtmp/rtsp
	 * @property {number} errRetryTimes   - 直播推流异常时，重试次数
     */
	/**
     * @typedef {object} CRVideo_MutiMixerOutputsObj -  获混图器内容配置的参数,json结构体数组
     * @property  {number} id  - 混图器编号
	 * @property {[{},{}]} output 混图器内容配置，参见CRVideo_MixerOutputObj
     */
	/**
	 * 开启本地录制、直播推流
	 * @access public
	 * @param {string} mixerID  - 混图器唯一标识,，请确保创建了有效的混图器createLocMixer
	 * @param {CRVideo_MixerOutputObj} mixerOutput  - 本地输出对象，json格式,CRVideo_MixerOutputObj
	 * @returns {number} 返回 错误码
	 * 
	 */
	CRVideo_AddLocMixerOutput = function(mixerID, mixerOutput)
	{
	    return CRVideo._crMeet.addLocMixerOutput(mixerID,JSON.stringify(mixerOutput));
	}
	/**
	 * 停止本地录制、直播推流 
	 * @access public
	 * @param {string} mixerID  - 混图器唯一标识
	 * @param {string} nameOrUrl - 要停止的录制文件名或直播url，支持多值，以";"分隔；
	 * @returns 无
	 */
	CRVideo_RmLocMixerOutput = function(mixerID,nameOrUrl)
	{
	    CRVideo._crMeet.rmLocMixerOutput(mixerID,nameOrUrl);
	}
	/**
	 * 开始云端录制、云端直播 
	 * @access public
	 *@param {CRVideo_MutiMixerCfgsObj} mutiMixerCfgs  - 服务器混图配置，定义见CRVideo_MutiMixerCfgsObj
	 * @param {CRVideo_MutiMixerContentsObj} mutiMixerContents  - 服务器混图内容，定义见CRVideo_MutiMixerContentsObj
	 *@param {CRVideo_MutiMixerOutputsObj} mutiMixerOutputs  - 服务器输出，定义见CRVideo_MutiMixerOutputsObj
	 * @returns {number} 返回 错误码
	 * 
	 */
	CRVideo_StartSvrMixer = function(mutiMixerCfgs, mutiMixerContents,mutiMixerOutputs)
	{
	    return CRVideo._crMeet.startSvrMixer(JSON.stringify(mutiMixerCfgs),JSON.stringify(mutiMixerContents),JSON.stringify(mutiMixerOutputs));
	}
/**
	 * 更新云端录制、云端直播内容 
	 * @access public
	 * @param {CRVideo_MutiMixerContentsObj} mutiMixerContents  - 服务器混图内容，定义见MutiMixerContentsObj
	 * @returns {number} 返回 错误码
	 * 
	 */
	CRVideo_UpdateSvrMixerContent = function(mutiMixerContents)
	{
	    return CRVideo._crMeet.updateSvrMixerContent(JSON.stringify(mutiMixerContents));
	}
	/**
	 *  停止云端录制、云端直播 
	 * @access public
	 ...
	 * @returns 无 
	 */
	CRVideo_StopSvrMixer = function()
	{
	   	CRVideo._crMeet.stopSvrMixer();
	}
	/**
	 * 获取云端录制、云端直播状态
	 * @access public
	 ...
	 * 
	 */
	CRVideo_GetSvrMixerState = function()
	{
	    return CRVideo._crMeet.getSvrMixerState();
	}
	/**
	 * 设置某个用户的昵称
	 * @access public
	 * @param {string} userid  - 用户ID
	 * @param {string} strNickName  - 新的用户昵称 
	 * @returns 无
	 * 
	 */
	CRVideo_SetNickName = function(userid,strNickName)
	{
	    CRVideo._crMeet.setNickName(userid,strNickName);
	}
	 ...
	 * @param {CRVideo_VIDEOLAYOUTMODE} videoWallMode - 分屏模式
	 */
	CRVideo_SetVideoWallMode2 = function(videoWallMode)
	{
		CRVideo._crMeet.setVideoWallMode2(videoWallMode);
	}
	/**
	 * 设置视频墙分屏模式2
	 * @access public
	 * @return {CRVideo_VIDEOLAYOUTMODE} videoWallMode - 分屏模式
	 */
	CRVideo_GetVideoWallMode2 = function()
	{
		return CRVideo._crMeet.getVideoWallMode2();
	}
	/**
	 * 获取录制文件信息
	 * @access public
	 * @param {string} fileName - 文件名， 不含路径
	 * @return {CRVideo_RecordFileInfo} json格式,详见CRVideo_RecordFileInfo
	 */
	CRVideo_GetRecordFileInfo = function(fileName)
	{
		return JSON.parse(CRVideo._crMeet.getRecordFileInfo(fileName));
	}
/**
	 * 剔出用户
	 * @access public
	 * @param {string} userId - 用户id
	 */
	CRVideo_Kickout = function(userId)
	{
		CRVideo._crMeet.kickout(userId);
	}
	...
	//-------------------------------------------
	/**
	 * @typedef {object} CRVideo_FileInfo - Http文件传输信息(params详细说明：decodeCREEFile：取值0或1。此参数仅上传有效，为0时上传原始文件，为1时上传解密的文件)
	 ...
	 * @property {object} extHeaders - http头部扩展信息，字典数据。
	 ...
	 * @access public
	 */
	CRVideo_StopMgr = function()
	{
		 CRVideo._crHttp.stopMgr();
	}
	/* 停止http文档上传下载功能
	 * @access public
	 */
	CRVideo_StopHttpMgr = function()
	{
		 CRVideo._crHttp.stopMgr();
	}
	/**
	 * 获取本地所有上传、下载文件信息
	 * @access public
	 * @returns {CRVideo_HttpFileInfoObjs} filelist - 返回文件信息列表
	 */
	 ...
	 * @access public
	 * @property {string} filePathName - 本地完整路径文件名(路径中要求有“CloudroomVideoSDK”)
	 * @returns {CRVideo_HttpFileInfoObj} fileinfo - 返回文件信息
	 */
	 ...
	 * 开始下载/上传文件
	 * @access public
	 * @property {CRVideo_HttpReqInfoObj} fileinfo - 格式的文件信息
	 */
	...
	CRVideo_CreatVideoObj = function()
	{
		...
		video.dblClickFullScreen(CRVideo.dblClickFullScreen);
		return video;
	}
	/**
	 * 创建media对象
	...
	CRVideo_CreatMediaObj = function()
	{
		...
		media.dblClickFullScreen(CRVideo.dblClickFullScreen);
		return media;
	}
	/**
	 * 创建screenShare对象
	 ...
	 * @access public
	 */
	CRVideo_CreatBoardObj = function() {
		var handler;
		if(CRVideo_Is_ActiveXObject) {
			if(document.getElementById("crBoardObj")){
				handler = document.getElementById("crBoardObj");
			}
		} else {
			if(document.getElementById("crBoardObjG")){
				handler = document.getElementById("crBoardObjG")
			}
		}
		var board = new CRVideo.BoardContainer();
